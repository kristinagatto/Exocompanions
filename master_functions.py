...
the
    orbiting exoplanet, and inclination and returns the 
semi-amplitude of 
    the exoplanet.
    ...
    two = ((planet_mass)*(np.sin(inclination))) / ((star_mass + 
planet_mass)**(2/3))
...
def RadialVelocity1(m_1,q,e,i,a,phi,w,t,eps):
    ...
    eccentric_anomaly, cos_true_anomaly, sin_true_anomaly = 
kepler.kepler(mean_anomaly, e)
    k = 
np.sqrt(c.G/(((m_1+m_2)*c.M_sun)*(a*c.au)*(1-e**2)))*(m_2*c.M_sun)*np.sin(np.deg2rad(i))
    b = (np.cos(w) * cos_true_anomaly - np.sin(w) * sin_true_anomaly 
+ e * np.cos(w))
    
    rv = k*b
    #print(rv)
    RV = (rv).values
    #sigma = np.array(np.random.uniform(low=2, high=3,size=1), 
dtype=float)
...
def f_value(a1,a2):
    ...
    funct = (np.cos(psi)) / ((1 - 
(2*(a1/a2))*(np.cos(psi))+((a1/a2)**2))**(3/2))
...
def f2_sci(psi, a1, a2):
    ...
    return (np.cos(2*psi)) / ((1 - 
(2*(a1/a2))*(np.cos(psi))+((a1/a2)**2))**(3/2))

def a_22(m1,m2,big_m,a1,a2,f):
    T = 
np.sqrt(4*(pi**2)*((a2*1.496E11)**3)/(G*((big_m*(1.89)*10E27)+(m2*(1.89)*10E27))))
    n_2 = 2*pi/T
    one = (n_2) * (1/(4*pi))
    two = m1 / (big_m + m2)
    three = ((a1/a2)**2) * f
...
def 
f_ll(a22,a21,e1ic,e2ic,omega1,omega2,b21,i1ic,i2ic,big_omega1,big_omega2,b22):
...
def e3_c(big_m,m1,m2,m3,a2,a3,fll):
    ...
    one = 15/16
    two = 
((m3*(1.89)*10E27)*(np.sqrt(G)))/(np.sqrt((big_m*(1.89)*10E27) + 
(m1*(1.89)*10E27) + (m2*(1.89)*10E27)))
